...
const iteration = (newHazard: Position, maxLocationsSize: number): boolean => {
  ...
  for (let y = 0; y<lines.length; y++) {
    ...
    for (let x = 0; x < chars.length; x++) {
      ...
      if (x === newHazard.x && y === newHazard.y && char !== '^') {
        row.push('#')
      } else {
        if (char === '#') {
  ...
  const pastVisitedSetSizes: Array<number> = [];

  const detectLoop = (): boolean => {
    pastVisitedSetSizes.push(visitedCoordinates.size);

    if (pastVisitedSetSizes.length > maxLocationsSize) {
      pastVisitedSetSizes.shift();
    } else {
      return false;
    }

    // we want to see if we've hit a loop
    // we can do this by checking if the last 20 visited set sizes are the same
    let areAllTheSame = true;

    for (let i = 0; i < pastVisitedSetSizes.length - 1; i++) {
      for (let j = i + 1; j < pastVisitedSetSizes.length; j++) {
        if (pastVisitedSetSizes[i] !== pastVisitedSetSizes[j]) {
          areAllTheSame = false;
          break;
        }
      }
  ...
  while (inPatrolArea) {
    ...
    if (loop) {
      console.log(`Loop detected after ${iterations} iterations`);
      return true;
    }
  }
...
let loopingIterations = 0;

for (let i = 0; i<130; i++) {
  for (let j = 0; j<130; j++) {
    const loops = iteration({ x: i, y: j }, 1000);
    if (loops) {
      loopingIterations++;
    }
  }
...
console.log(`Looping iterations: ${loopingIterations}`);